---
title: 技术快速参考卡
summary: 常见命令、配置模板与协作约定，适合作为团队内部的速查清单。
...
spring:
  ...
  datasource:
    url: jdbc:oracle:thin:@db.example.com:1521:CUSTOMER
...
logging:
  ...
  file:
    name: /var/log/example/application.log
...
| 名称 | 链接 |
|------|------|
| GitLab | gitlab.hsbc.com（示例） |
| Jira | jira.hsbc.com（示例） |
| Confluence | confluence.hsbc.com（示例） |
| Artifactory | artifactory.hsbc.com（示例） |
| Grafana | grafana.hsbc.com（示例） |

> ⚠️ [修订注：以上 `*.hsbc.com` 为示意性的域名格式，未经核实，未必是汇丰真实内部入口。请以入职后实际分配的内部地址为准。]
