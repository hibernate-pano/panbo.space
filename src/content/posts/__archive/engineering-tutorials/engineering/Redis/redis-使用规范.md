---
title: Redis 使用规范：高并发场景实践要点
summary: 生产环境 Redis 常见规范：键设计、命令选择、内存管理、连接池配置、监控告警，以及高合规场景下的安全要求。
...
slug: redis-使用规范
legacyPaths:
  - /coding/Redis/Redis 使用规范
---
> "Redis 用不对，轻则性能下降，重则引发数据不一致。"

## 前言

Redis 在高并发业务中常承担**热点数据缓存、分布式锁、会话存储、实时排行榜**等职责。用错了，轻则响应变慢，重则出现数据不一致甚至生产事故。以下规范主要基于通用工程经验整理。
